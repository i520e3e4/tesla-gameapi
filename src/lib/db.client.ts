...
// ---- 常量 ----
const PLAY_RECORDS_KEY = 'teslatv_play_records';
const FAVORITES_KEY = 'teslatv_favorites';
const SEARCH_HISTORY_KEY = 'teslatv_search_history';

// 缓存相关常量
const CACHE_PREFIX = 'teslatv_cache_';
...
export async function getSkipConfig(
  ...
  // localStorage 模式
  try {
    const raw = localStorage.getItem('teslatv_skip_configs');
    if (!raw) return null;
...
export async function saveSkipConfig(
  ...
  try {
    ...
    configs[key] = config;
    localStorage.setItem('teslatv_skip_configs', JSON.stringify(configs));
    window.dispatchEvent(
      new CustomEvent('skipConfigsUpdated', {
        detail: configs,
      })
