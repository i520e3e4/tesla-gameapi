...
- 🔄 **自动资源更新**：每天自动同步最新 API 配置，确保视频源始终可用。
- 📄 **丰富详情页**：支持剧集列表、演员、年份、简介等完整信息展示。
- ▶️ **流畅在线播放**：集成 HLS.js & ArtPlayer。
- 🌐 **CORS 代理**：内置 API 代理功能，解决被墙或跨域限制的资源访问问题。
- 🚗 **特斯拉车机优化**：独创 Gamepad API "欺骗"技术，完美解决特斯拉行驶时视频锁定问题。
...
本项目集成了独创的 **Gamepad API "欺骗"技术**，完美解决了特斯拉车机在行驶时无法播放视频的限制问题。

### 🔧 技术原理

- **核心技术**：通过持续调用浏览器的 `navigator.getGamepads()` API，模拟游戏手柄连接状态
- **系统欺骗**：让特斯拉车机系统误认为当前应用是"游戏"而非"视频应用"
- **绕过限制**：成功绕过特斯拉针对视频应用的行驶时锁定机制
- **稳定可靠**：基于底层浏览器API，不受特斯拉软件更新影响

### 📱 使用方法

1. **直接访问**：在特斯拉车机浏览器中打开本应用
2. **自动激活**：Gamepad API 监听会在页面加载时自动启动
3. **正常使用**：无需任何额外操作，即可在行驶中正常观看视频
4. **状态指示**：开发环境下会显示特斯拉优化状态指示器

### ⚡ 性能优化

- **智能轮询**：优化的轮询频率，平衡性能与效果
- **环境检测**：自动识别特斯拉浏览器环境
- **防休眠机制**：多重策略防止页面进入休眠状态
- **资源管理**：页面不可见时自动暂停优化，节省资源

### 🛡️ 兼容性说明

- ✅ **特斯拉 Model S/3/X/Y**：完全支持
- ✅ **最新车机系统**：持续测试兼容
- ✅ **所有视频源**：不限制视频来源和格式
- ⚠️ **其他车机**：理论支持，但未经测试
...
详细的技术实现和原理说明请参考：[TESLA_GAMEPAD_SOLUTION.md](./TESLA_GAMEPAD_SOLUTION.md)
